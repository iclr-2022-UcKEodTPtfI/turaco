...
for (name, n) in inputs:
    for i in range(n//3+1):
        fname = 'base-logger-gen-{}-{}.frag'.format(name, i)
        print('"{}",'.format(fname))
        with open(fname, 'w') as f:
            f.write(template)
            f.write('out0 = vec4({}, 1);}}}}'.format(
                '{}.xyz' if n == 4 and i == 0 else
                '{}.w, 0, 0' if n == 4 and i == 1 else
                name if n == 3 else
                '{}, 0' if n == 2 else
                '{}, 0, 0'
            ).format(name))
