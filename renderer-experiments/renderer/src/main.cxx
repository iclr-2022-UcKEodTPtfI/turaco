...
struct NN nn_data[9];
PT(ShaderBuffer) nn_data_buffer;
...
( int argc
  ...
  std::string scene = "base_day";
  int n_frames_to_exit_after = -1;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--log") {
      ...
      if (i >= argc + 1) {
        std::cerr << "--frames requires an argument" << std::endl;
        return 1;
      }
      n_frames_to_exit_after = atoi(argv[++i]);
    } else if (arg == "--screenshot") {
      do_screenshot = true;
    } else if (arg == "--synchronize") {
      do_synchronize = true;
    } else if (arg == "--surrogate") {
      if (i >= argc + 1) {
        std::cerr << "--surrogate requires an argument" << std::endl;
        return 1;
      }
      surrogate_name = argv[++i];
      is_surrogate = true;
    } else if (arg == "--scene") {
      if (i >= argc + 1) {
        std::cerr << "--scene requires an argument" << std::endl;
        return 1;
      }
      scene = argv[++i];
      if (scene == "base_day") {
      } else if (scene == "base_night") {
        sunlightP = 100;
      } else if (scene == "top_day") {
        sunlightP = 290;
        cameraRotatePhiInitial = 46.4131;
        cameraRotateThetaInitial = 45.3014;
        cameraRotateRadiusInitial = 1100.83;
        cameraLookAtInitial = LVecBase3(1.00839, 1.20764, 5.85055);
      } else if (scene == "top_night") {
        sunlightP = 1;
        cameraRotatePhiInitial = 46.4131;
        cameraRotateThetaInitial = 45.3014;
        cameraRotateRadiusInitial = 1100.83;
  ...
  int shuttersAnimationFrame = 0;
  int weatherVaneAnimationFrame = 0;
  int bannerAnimationFrame = 0;
  ...
  generateLights(render, sunlightP, false);

  PT(Shader) discardShader               = loadShader("discard", "discard");
  PT(Shader) baseShader                  = loadShader("base",    is_surrogate ? "surrogate" : "base");
  ...
  framebufferTextureArguments.aux_use_float = false;
  ...
  if (is_surrogate) {
    std::fstream fh;
    fh.open(surrogate_name, std::fstream::in | std::fstream::binary);
    fh.read((char*)&nn_data, sizeof(nn_data));
    fh.close();

    auto uchr = vector_uchar((uchar*) &nn_data, (uchar*) &nn_data + sizeof(nn_data));
    PT(ShaderBuffer) nn_data_buffer = new ShaderBuffer("nn_data", uchr, GeomEnums::UH_static);

    baseNP.set_shader_input("nn_data", nn_data_buffer);
  }
  ...
  std::vector<PT(Texture)> loggedTextures;
  std::vector<NodePath> loggedNPs;
  std::vector<PT(Camera)> loggedCameras;

  std::vector<std::string> fnames;

  if (do_log) {
    fnames.insert(fnames.end(), {
        "base-logger-gen-rimLight-0",
        "base-logger-gen-rimLight-1",
        "base-logger-gen-celShadingEnabled-0",
        "base-logger-gen-sunPosition-0",
        "base-logger-gen-gamma-0",
        "base-logger-gen-worldNormal-0",
        "base-logger-gen-ssao-0",
        "base-logger-gen-diffuseColor-0",
        "base-logger-gen-diffuseColor-1",
        "base-logger-gen-diffuse-0",
        "base-logger-gen-diffuse-1",
        "base-logger-gen-specular-0",
        "base-logger-gen-specular-1",
        "base-logger-gen-p3d_Material.emission.xyz-0",
        "base-logger-gen-isWater-0",
        "base-logger-gen-isParticle-0",
        "base-logger-out0-0",
        "base-logger-out0-1",
        "base-logger-out1-0",
        "base-logger-out1-1",
  ...
  for (auto shaderName : fnames) {
    PT(Shader) qaseShader                  = loadShader("base",    shaderName);

    FramebufferTextureArguments fTA;
    fTA.window         = window;
    fTA.graphicsOutput = graphicsOutput;
    fTA.graphicsEngine = graphicsEngine;

    fTA.bitplane       = GraphicsOutput::RTP_color;
    fTA.clearColor     = LColor(0, 0, 0, 0);
    fTA.setFloatColor  = true;
    fTA.setSrgbColor   = false;
    fTA.setRgbColor    = true;

    fTA.rgbaBits = rgba32;
    fTA.aux_rgba = 4;
    fTA.aux_use_float = true;
    fTA.useScene = true;
    fTA.name     = shaderName;

    FramebufferTexture qaseFramebufferTexture =
      generateFramebufferTexture
      ( fTA
        );
    PT(GraphicsOutput) qaseBuffer = qaseFramebufferTexture.buffer;
    PT(Camera)         qaseCamera = qaseFramebufferTexture.camera;
    NodePath           qaseNP     = qaseFramebufferTexture.shaderNP;

    framebufferTextureArguments.aux_use_float = false;

    qaseBuffer->add_render_texture
      ( NULL
        , GraphicsOutput::RTM_copy_ram
        , GraphicsOutput::RTP_aux_rgba_0
        );
    qaseBuffer->set_clear_active(3, true);
    qaseBuffer->set_clear_value( 3, framebufferTextureArguments.clearColor);
    ...
    qaseBuffer->set_clear_active(4, true);
    qaseBuffer->set_clear_value( 4, framebufferTextureArguments.clearColor);
    ...
    qaseBuffer->set_clear_active(5, true);
    qaseBuffer->set_clear_value( 5, framebufferTextureArguments.clearColor);
    ...
    qaseBuffer->set_clear_active(6, true);
    qaseBuffer->set_clear_value( 6, framebufferTextureArguments.clearColor);
    ...
    qaseBuffer->set_sort(0);
    qaseNP.set_shader(qaseShader);
    qaseNP.set_shader_input("pi",                PI_SHADER_INPUT);
    qaseNP.set_shader_input("gamma",             GAMMA_SHADER_INPUT);
    qaseNP.set_shader_input("ssaoBlurTexture",   ssaoBlurTexture);
    qaseNP.set_shader_input("flowTexture",       stillFlowTexture);
    qaseNP.set_shader_input("normalMapsEnabled", normalMapsEnabled);
    qaseNP.set_shader_input("blinnPhongEnabled", blinnPhongEnabled);
    qaseNP.set_shader_input("fresnelEnabled",    fresnelEnabled);
    qaseNP.set_shader_input("rimLightEnabled",   rimLightEnabled);
    qaseNP.set_shader_input("celShadingEnabled", celShadingEnabled);
    qaseNP.set_shader_input("flowMapsEnabled",   flowMapsEnabled);
    qaseNP.set_shader_input("specularOnly",      LVecBase2f(0, 0));
    qaseNP.set_shader_input("isParticle",        LVecBase2f(0, 0));
    qaseNP.set_shader_input("isWater",           LVecBase2f(0, 0));
    qaseNP.set_shader_input("sunPosition",       LVecBase2f(sunlightP, 0));

    qaseCamera->set_initial_state(qaseNP.get_state());
    qaseCamera->set_tag_state_key(shaderName);
    qaseCamera->set_tag_state("isParticle", isSmokeNP.get_state());
    qaseCamera->set_tag_state("isWater",    isWaterNP.get_state());
    smokeNP.set_tag(shaderName, "isParticle");
    waterNP.set_tag(shaderName, "isWater");
    qaseCamera->set_camera_mask(BitMask32::bit(6));

    auto m_texture = qaseBuffer->get_texture(1);
    m_texture->set_compression(Texture::CM_off);
    loggedTextures.push_back(m_texture);
    loggedNPs.push_back(qaseNP);
    loggedCameras.push_back(qaseCamera);
    // loggedTextures.push_back(qaseBuffer->get_texture(2));
    // loggedTextures.push_back(qaseBuffer->get_texture(3));
    // loggedTextures.push_back(qaseBuffer->get_texture(4));
  }
  ...
  float microseconds_per_step = 0.03;
  ...
  auto mtime = std::chrono::duration_cast
    <std::chrono::microseconds>
    ( std::chrono::system_clock::now().time_since_epoch()
      ).count();;

  std::ostringstream textureFolderStringStream;
  textureFolderStringStream << "textures_" << mtime;
  auto texture_folder_name = textureFolderStringStream.str();

  std::ostringstream screenshotFolderStringStream;
  screenshotFolderStringStream << "screenshots_" << mtime;
  auto screenshot_folder_name = screenshotFolderStringStream.str();

  if (do_screenshot || do_log) {
    std::cout << mtime << std::endl;
  }

  if (do_screenshot) {
    mkdir(screenshot_folder_name.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  }

  if (do_log) {
    mkdir(texture_folder_name.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  }
  ...
  auto beforeFrame =
    [&]() -> void {
      ...
      /*
        std::cout <<
        "sunlightP: " << sunlightP <<
        ", cameraRotateRadius: " << cameraRotateRadius <<
        ", cameraRotatePhi: " << cameraRotatePhi <<
        ", cameraRotateTheta: " << cameraRotateTheta <<
        ", cameraLookAt: " << cameraLookAt <<
        std::endl;
      */

      if (do_synchronize) {
        delta = microseconds_per_step;
        shuttersAnimationCollection.pose(shuttersAnimationCollection.which_anim_playing(), shuttersAnimationFrame);
        weatherVaneAnimationCollection.pose(weatherVaneAnimationCollection.which_anim_playing(), weatherVaneAnimationFrame);
        bannerAnimationCollection.pose(bannerAnimationCollection.which_anim_playing(), bannerAnimationFrame);

        shuttersAnimationFrame = (shuttersAnimationFrame + frames_per_step) % shuttersAnimationCollection.get_num_frames();
        weatherVaneAnimationFrame = (weatherVaneAnimationFrame + frames_per_step) % weatherVaneAnimationCollection.get_num_frames();
        bannerAnimationFrame = (bannerAnimationFrame + frames_per_step) % bannerAnimationCollection.get_num_frames();
      }
      ...
      for (int npidx = 0; npidx < loggedNPs.size(); npidx++) {
        auto np = loggedNPs[npidx];
        auto cam = loggedCameras[npidx];

        np.set_shader_input("sunPosition",       LVecBase2f(sunlightP, 0));
        cam->set_initial_state(np.get_state());
      }
      ...
      if (do_screenshot) {
        std::ostringstream s;
        s << screenshot_folder_name << "/" << frame_idx << ".png";
        graphicsOutput->save_screenshot(s.str());
      }

      if (do_log) {
        int textureLog_Idx = 0;
        for (auto texture : loggedTextures) {
          std::ostringstream s;
          s << texture_folder_name << "/texture_id_" << textureLog_Idx << "_it_" << frame_idx << ".tex";
          auto ram_image = texture->get_ram_image_as("RGBA");
          std::ofstream file(s.str(), std::ios::binary);
          file.write((char*)ram_image.p(), texture->get_ram_image_size());
          textureLog_Idx += 1;
        }
      }
      ...
      if (n_frames_to_exit_after > 0) {
        int barWidth = 70;
        float progress = ((float) frame_idx) / (float) n_frames_to_exit_after;
        ...
        if (frame_idx >= n_frames_to_exit_after) {
          std::cerr << std::endl;
          exit(0);
        }
      }
...
( NodePath render
  ...
  sunlightPivotNP.set_hpr(135, sunlightP, 0);
  ...
  moonlightPivotNP.set_hpr(135, sunlightP - 180, 0);
...
( FramebufferTextureArguments framebufferTextureArguments
  ...
  if (framebufferTextureArguments.aux_use_float) {
    fbp.set_aux_float(aux_rgba);
  } else {
